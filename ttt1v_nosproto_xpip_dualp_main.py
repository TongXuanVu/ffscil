...
from piplib.ffscil_xpip_engine1v_nosproto import *
    ...
    # for task_id in range(1,args.num_tasks):
        ...
        for n_round in range(0,RT):
            ...
            if (task_id == 0):
                ...
                if (n_round) == 0:
                    clients_weight =  [clients_participations[i] for i in clients_index]
                    clients_prototype, clients_prototype_var = generate_prototype_only([models[i] for i in clients_index], 
                                    [models_without_ddp[i] for i in clients_index], 
                                    [original_models[i] for i in clients_index],
                                    criterion, 
                                    [data_loaders[i] for i in clients_index],
                ...
                clients_prototype, clients_prototype_var  = train_fs_pertask_v2([models[i] for i in clients_index], 
                                    [models_without_ddp[i] for i in clients_index], 
                                    [original_models[i] for i in clients_index],
                                    criterion, 
                                    [data_loaders[i] for i in clients_index],
                                    ...
                                    device, 
                                    [class_masks[i] for i in clients_index], 
                                    task_id, global_prototype, global_prototype_var, 
                                    global_prototype, global_prototype_var ,args)
                print("Train 1 round with prototype done")
        ...
        if(task_id==0):
            ...
            # fixed_g_prompt = copy.deepcopy(server_model.g_prompt)
            # print(fixed_FC_dict)
            # print(fixed_FC_dict2)
            # print(fixed_g_prompt)
            server_optimizer = create_optimizer(args, server_model_without_ddp)
        else:
            # print("Reload head dict")
