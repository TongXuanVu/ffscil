...
def train_one_epoch_with_prototypes(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, max_norm: float = 0,
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if iter == 0:        
            ...
            proto1 = proto + (d*proto_std)
            proto2 = proto + (-1.0*d*proto_std)
            d =  torch.rand(proto.shape, dtype=proto.dtype).to(device, non_blocking=True) / 2.0 
            proto3 = proto + (d*proto_std)
            proto4 = proto + (-1.0*d*proto_std)

            # proto = torch.cat([proto,proto1,proto2],dim=0)
            # proto_label = torch.cat([proto_label,proto_label,proto_label],dim=0)
            proto = torch.cat([proto,proto1,proto2,proto3,proto4],dim=0)
            proto_label = torch.cat([proto_label,proto_label,proto_label,proto_label,proto_label],dim=0)


        # print("cls features shape:")
        # print(cls_features.shape)
        # output = model(input, task_id=task_id, cls_features=cls_features, train=set_training_mode)
        # print(cls_features.shape)
        output = model.forward_with_proto(input, task_id=task_id, cls_features=cls_features, proto=proto, train=set_training_mode)
        ...
        # logits2 = model.head(proto)
        # loss2 = criterion(logits2, proto_label) # base criterion (CrossEntropyLoss)        
...
def train_pertask_wo_prototypes(models, models_without_ddp, original_models, 
                    criterion, data_loaders, optimizers, lr_schedulers, device: torch.device, 
                    class_masks=None, task_id=0, args = None,):
    ...
    # clients_prototype_var = []
    # for n in range(len(models)):            
    #     prototype_dict, prototype_var =  get_prototype(model=models[n], original_model=original_models[n], criterion=criterion, 
    #                                 data_loader=data_loaders[0][task_id]['train'], optimizer=optimizers[n], 
    #                                 device=device, epoch=args.epochs, max_norm=args.clip_grad, 
    #                                 set_training_mode=True, task_id=task_id, class_mask=class_masks[n], args=args,)
    ...
    # return clients_prototype[0], clients_prototype_var[0]


def train_pertask_with_prototypes(models, models_without_ddp, original_models, 
                    criterion, data_loaders, optimizers, lr_schedulers, device: torch.device, 
    ...
    for epoch in range(args.epochs):
        for n in range(len(models)):            
            train_stats = train_one_epoch_with_prototypes(model=models[n], original_model=original_models[n], criterion=criterion, 
                                        data_loader=data_loaders[0][task_id]['train'], optimizer=optimizers[n], 
                                        device=device, epoch=epoch, max_norm=args.clip_grad, 
                                        set_training_mode=True, task_id=task_id, class_mask=class_masks[n], global_prototype=global_prototype, global_prototype_var=global_prototype_var, args=args,)
...
def get_prototype(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, max_norm: float = 0,
                    set_training_mode=True, task_id=-1, class_mask=None, args = None,):
    ...
    min_c = task_id*args.classes_per_task
    max_c = (task_id+1)*args.classes_per_task
    cl_data = [0 for i in range(max_c+1)]

    iter=0
    for input, target in data_loader:
        list_classes = list(set(target.tolist()))
        ...
        for c in list_classes:
            ...
            if(cl_data[c]==0):
                ...
                # prototype_dict[c] = torch.mean(pl_class, dim=0).cpu().detach().numpy()
                # prototype_var[c] = torch.var(pl_class, dim=0).cpu().detach().numpy()
                prototype_dict[c] = np.mean(pl_class.cpu().detach().numpy(),axis=0)
                prototype_var[c] = np.var(pl_class.cpu().detach().numpy(),axis=0)
                ...
                mu1 =  prototype_dict[c]
                # mu2 =  torch.mean(pl_class,dim=0).cpu().detach().numpy()
                mu2 = np.mean(pl_class.cpu().detach().numpy(),axis=0)
                var1 = prototype_var[c]
                # var2 = torch.var(pl_class, dim=0).cpu().detach().numpy()
                var2 = np.var(pl_class.cpu().detach().numpy(),axis=0)
                n1 = cl_data[c]
                n2 = pl_class.shape[0]

                prototype_dict[c] = ((n1*mu1)+(n2*mu2))/(n1+n2)
                prototype_var[c] = ((((var1+(mu1*mu1))*n1) + ((var2+(mu2*mu2))*n2))/(n1+n2)) - (prototype_dict[c]*prototype_dict[c])
                # print(prototype_var[c])
                # prototype_dict[c] = ((cl_data[c] * prototype_dict[c]) + (torch.mean(pl_class,dim=0).cpu().detach().numpy() * pl_class.shape[0]))/(cl_data[c]+pl_class.shape[0])
                # prototype_var[c] = ((cl_data[c] * prototype_dict[c]) + (torch.mean(pl_class,dim=0).cpu().detach().numpy() * pl_class.shape[0]))/(cl_data[c]+pl_class.shape[0])
