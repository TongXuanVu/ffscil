        ...
        if task_id > 0:
            ...
            if (prev_end > args.size) or (cur_end > args.size):
                ...
                for n in range(len(models)): 
                    cur_idx = (slice(None), slice(None), slice(cur_start, cur_end)) if args.use_prefix_tune_for_prompt else (slice(None), slice(cur_start, cur_end))
                    prev_idx = (slice(None), slice(None), slice(prev_start, prev_end)) if args.use_prefix_tune_for_prompt else (slice(None), slice(prev_start, prev_end))

                    with torch.no_grad():
                        if args.distributed:
                            # models[n].module.prompt.prompt.grad.zero_()
                            models[n].module.prompt.prompt[cur_idx] = models[n].module.prompt.prompt[prev_idx]
                            optimizers[n].param_groups[0]['params'] = models[n].module.parameters()
                        else:
                            # models[n].prompt.prompt.grad.zero_()
                            models[n].prompt.prompt[cur_idx] = models[n].prompt.prompt[prev_idx]
    ...
    if args.prompt_pool and args.shared_prompt_key:
        if task_id > 0:
            ...
            for n in range(len(models)):
                with torch.no_grad():
                    if args.distributed:
                        # models[n].module.prompt.prompt_key.grad.zero_()
                        models[n].module.prompt.prompt_key[cur_idx] = models[n].module.prompt.prompt_key[prev_idx]
                        optimizers[n].param_groups[0]['params'] = models[n].module.parameters()
                    else:
                        # models[n].prompt.prompt_key.grad.zero_()
                        models[n].prompt.prompt_key[cur_idx] = models[n].prompt.prompt_key[prev_idx]
                        optimizers[n].param_groups[0]['params'] = models[n].parameters()
