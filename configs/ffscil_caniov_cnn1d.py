...
def get_args_parser(subparser):
    """Cau hinh FFSCIL cho bo CAN-bus / IoV 100 client.

    Sao tu ffscil_cicio23_cnn1d.py, chi doi cac tham so phu thuoc bo du lieu.
    Moi sieu tham so hoc (lr, batch, prompt, optimizer) giu Y NGUYEN de so sanh
    IoT vs IoV la so sanh cong bang.

    Khac biet so voi ban IoT:
      num_tasks        6  -> 5
      nb_classes      36  -> 13   (IoV khong can pad: xem ghi chu ben duoi)
      task_increments  6,6,6,6,5,5 -> 3,3,3,2,2
      input-size      33  -> 31
      base/fs_classes  6  -> 3
      dataset      cic_iot23 -> can_iov
    """
    ...
    # IoV co dung 13 lop, KHONG pad. Ban IoT pad 34 -> 36 chi de tuong thich
    # nguoc voi checkpoint task 1-4 da train truoc do; o day chua co checkpoint
    # nao nen khong can, va pad thua se lam mau so macro-F1 sai.
    subparser.add_argument('--nb_classes', default=13, type=int)
    # Khop cach chia that cua bo IoV 100 client (xem CLAUDE.md va
    # AFSIC-IOV/tools/make_fewshot_iov100.py). KHONG suy tu cong thuc deu.
    subparser.add_argument('--task_increments', default='3,3,3,2,2', type=str,
                           help='So lop moi task, cach nhau bang dau phay (khop data split)')
    subparser.add_argument('--num_clients', default=100, type=int)
    subparser.add_argument('--local_clients', default=100, type=int)
    subparser.add_argument('--rounds_per_task', default=30, type=int)
    subparser.add_argument('--base_classes', default=3, type=int)
    subparser.add_argument('--fs_classes', default=3, type=int)
    subparser.add_argument('--fs_shots', default=5, type=int)
    subparser.add_argument('--available_base_classes', default=3, type=int)
    subparser.add_argument('--available_fs_classes', default=3, type=int)

    # Model parameters
    subparser.add_argument('--model', default='cnn1d_prompt', type=str, help='Name of model to train')
    subparser.add_argument('--input-size', default=31, type=int, help='so dac trung CAN-bus IoV')
    ...
    # Augmentation parameters (khong dung cho du lieu bang, giu de tuong thich)
    ...
    subparser.add_argument('--data-path', default='/kaggle/input/datasets/tongxuanvu/100clientiov', type=str, help='dataset path')
    subparser.add_argument('--dataset', default='can_iov', type=str, help='dataset name')
    subparser.add_argument('--output_dir', default='./output_iov_real', help='path where to save, empty for no saving')
