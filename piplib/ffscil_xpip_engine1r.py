    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if(global_prototype is None):
            ...
            if(iter == 0):
                query_proto = proto
                query_label = proto_label
                # query_proto_std = proto_std
                if(len(unava_proto_label) > 0):
                    for c in unava_proto_label:
                        proto_c = torch.mean(cls_features[target==c],dim=0).unsqueeze(0)
                        # if args.fs_shots == 1:
                        #     proto_c_std = torch.mean(proto_std,dim=0)
                        # else:
                        #     proto_c = torch.std(cls_features[target==c],dim=0).unsqueeze(0)
                        # print(proto_c.shape)
                        query_proto = torch.cat((query_proto,proto_c),dim=0)
                        # query_proto_std = torch.cat((query_proto_std,proto_c_std),dim=0)
                ...
                query_proto_kshot.append(query_proto)
                query_label_kshot.append(query_label)
                q_std = 0.0001 * torch.ones_like(query_proto).cuda()
                for i in range(1,fs_shots):
                    q_p = torch.normal(mean=query_proto, std=q_std).cuda()
