    ...
    for epo in range(0,args.epochs):
        ...
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            ...
            if target.shape[0]==0:
             ...
             # Supervision Noise
            idx = random.sample([*range(0, len(target), 1)], int(0.05 * len(target)) )
            # print(idx)
            t_noise = random.choices(available_classes, k=len(idx))
            for ii in range(0,len(idx)):
                target[idx[ii]] = t_noise[ii]
            ...
            # logits = logits + torch.normal(mean=torch.zeros(logits.shape),std=(torch.ones(logits.shape)*0.001)).cuda()
            ...
            if (iter > 0):
                # feat_mix, y_a, y_b, rep_lam, mixup_indexes = mixup_batch_representations(cls_features,target)
                # s_output = s_model(input, task_id=task_id, cls_features=feat_mix, train=set_training_mode)
                s_output = s_model(input, task_id=task_id, cls_features=cls_features, train=set_training_mode)
                s_logits = s_output['logits']

                if args.train_mask and class_mask is not None:
                    s_logits = s_logits.index_fill(dim=1, index=not_mask, value=float('-inf'))

                # loss = (0.5*loss) + (0.5*(rep_lam * criterion(s_logits, y_a) + (1-rep_lam) * criterion(s_logits, y_b)))
                loss = (0.5*loss) + criterion(s_logits, target)
            ...
            s_model.e_prompt.prompt.data = (gamma*s_model.e_prompt.prompt.data) + (1-gamma)*model.e_prompt.prompt.data 
            # s_model.head.weight.data = (gamma*s_model.head.weight.data) + (1-gamma)*model.head.weight.data
            # s_model.head.bias.data = (gamma*s_model.head.bias.data) + (1-gamma)*model.head.bias.data 
            s_model.head.load_state_dict(model.head.state_dict())
    del s_model
    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    print("Averaged stats:", metric_logger)
    ...
    for epo in range(0,args.epochs):
        ...
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            ...
            if (iter > 0):
                ...
                # y_a = torch.cat([y_a,proto_label],dim=0)
                # y_b = torch.cat([y_b,proto_label],dim=0)
...
def train_fs_all_epoch_with_available_classes_v2(model: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    # s_model = copy.deepcopy(model)
    s_e_prompt =  copy.deepcopy(model.e_prompt)
    gamma = 0.1

    for epo in range(args.fs_epochs):
        header = f'Train: Epoch[{epo+1:{int(math.log10(args.fs_epochs))+1}}/{args.fs_epochs}]'
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            tmp = copy.deepcopy(target)
            filter = tmp.apply_(lambda x: x in available_classes).bool()
            ...
            # logits = logits + torch.normal(mean=torch.zeros(logits.shape),std=(torch.ones(logits.shape)*0.01)).cuda()
            print(logits.shape)
            print(target.shape)
            ...
            if (iter > 0):
                e_prompt_ori_data = model.e_prompt.prompt.data
                model.e_prompt.prompt.data = s_e_prompt.prompt.data
                output, logit_query, opt_proto = model.forward_fs(input,  target1, query_proto_kshot, query_label_kshot, args.available_fs_classes, args.fs_shots, task_id, cls_features, True)
                pre_logits = output['pre_logits']
                # pre_logits, y_a, y_b, rep_lam, mixup_indexes = mixup_batch_representations(pre_logits,target)
                s_logits = None
                ...
                for j in range(0,pre_logits.shape[0]):
                    ...
                    if isFirst:
                        ...
                        s_logits=torch.cat((s_logits,logit), dim=0)

                # loss = (0.5*loss) + (0.5*(rep_lam * criterion(s_logits, y_a) + (1-rep_lam) * criterion(s_logits, y_b)))
                loss = (0.5*loss) + criterion(s_logits, target)
                model.e_prompt.prompt.data = e_prompt_ori_data
            ...
            s_e_prompt.prompt.data = (gamma*s_e_prompt.prompt.data) + (1-gamma)*model.e_prompt.prompt.data 
            # s_model.fs_head.load_state_dict(model.fs_head.state_dict())
            # for param1, param2 in zip(s_model.fs_head.parameters(), model.fs_head.parameters()):
            #     param1 = (gamma * param1) + (1-gamma)*param2
                # print(param2)
