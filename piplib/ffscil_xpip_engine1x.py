...
def train_all_epoch_with_available_classes(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, max_norm: float = 0,
    ...
    for epo in range(0,args.epochs):
        header = f'Train: Epoch[{epo+1:{int(math.log10(args.epochs))+1}}/{args.epochs}]'
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            # if task_id > 0:
            # print(set(target))
...
def train_all_epoch_with_available_classes_v2(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, max_norm: float = 0,
...
def train_pertask(models, models_without_ddp, original_models, 
    ...
    for n in range(len(models)):            
        ...
        train_stats = train_all_epoch_with_available_classes(model=models[n], original_model=original_models[n], criterion=criterion, 
...
def train_pertask_v2(models, models_without_ddp, original_models, 
    ...
    for n in range(len(models)):            
        ...
        train_stats = train_all_epoch_with_available_classes_v2(model=models[n], original_model=original_models[n], criterion=criterion, 
                                    data_loader=data_loaders[n][task_id]['train'], optimizer=optimizers[n], 
                                    device=device, epoch=args.epochs, max_norm=args.clip_grad, 
                                    set_training_mode=True, task_id=task_id, class_mask=class_masks[n], 
...
def train_fs_all_epoch_with_available_classes(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, max_norm: float = 0,
                    set_training_mode=True, task_id=-1, class_mask=None, available_classes=None, 
                    global_prototype=None, global_prototype_var=None,  args = None,):
...
def train_fs_all_epoch_with_available_classes_v2(model: torch.nn.Module, original_model: torch.nn.Module, 
                    criterion, data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, max_norm: float = 0,
                    set_training_mode=True, task_id=-1, class_mask=None, available_classes=None, 
    ...
    for epo in range(0,args.epochs):
        ...
        for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
            ...
            if iter == 0:
                ...
                if target.shape[0]==0:
                    print("Data is not found in loader")
                    break

                print(list(set(target.tolist())))
                if (args.fs_shots >= 5):
                    input1 = copy.deepcopy(input).cuda()
                    target0 = copy.deepcopy(target).cuda()
                    target1 = copy.deepcopy(target).apply_(lambda x: label_converter[x]).cuda()
                else:
                    input1 = copy.deepcopy(input)
                    target0 = copy.deepcopy(target)
                    n_append = fs_shots - args.fs_shots
                    for c in available_classes:
                        input_c = input[target==c]
                        target_c = target[target==c]
                        list_idx = [*range(0, input_c.shape[0], 1)] 
                        select_idx = random.choices(list_idx,k=n_append)
                        select_input = input_c[select_idx]
                        # select_label = torch.Tensor(([c]*n_append))
                        select_label = target_c[select_idx]
                        input1 = torch.cat((input1,select_input),dim=0)
                        target0 = torch.cat((target0,select_label),dim=0)

                    input1 = input1.cuda()
                    target1 = copy.deepcopy(target0).apply_(lambda x: label_converter[x]).cuda()
            ...
            if(iter ==0):
                ...
                if (all_global_prototype is not None):
                    ...
                    proto = torch.tensor(np.array([all_global_prototype[x] for x in ava_proto_label]), dtype=input1.dtype).to(device, non_blocking=True)
                    proto_std = torch.square(torch.tensor(np.array([all_global_prototype_var[x] for x in ava_proto_label]), dtype=input1.dtype).to(device, non_blocking=True))
                    proto_label = torch.tensor(np.array(ava_proto_label),dtype=target0.dtype).to(device, non_blocking=True)
                    ...
                    for k in all_global_prototype.keys():
                        all_proto[k] = torch.tensor(np.array(all_global_prototype[k]), dtype=input1.dtype).to(device, non_blocking=True)
                        all_proto_std[k] = torch.square(torch.tensor(np.array(all_global_prototype_var[k]), dtype=input1.dtype).to(device, non_blocking=True))

            if(global_prototype is None):
                query_proto = model.forward_get_prelogits(input1, task_id, cls_features, train=True)
                query_label = target1
                # if(args.fs_shots < 5):
                #     sz = query_proto.size()
                #     pmean = query_proto.clone().cuda()
                #     pstd = torch.ones_like(query_proto).cuda() * 0.005
                #     plabel = query_label.clone().cuda()
                #     for i in range(0, (fs_shots-args.fs_shots)):
                #         aug_proto = torch.normal(mean=pmean,std=pstd).cuda()
                #         query_proto=torch.cat((query_proto,aug_proto),dim=0)
                #         query_label=torch.cat((query_label,plabel),dim=0)
                ...
                if(iter == 0):
                    ...
                    if(len(unava_proto_label) > 0):
                        for c in unava_proto_label:
                            proto_c = torch.mean(cls_features[target0==c],dim=0).unsqueeze(0)
                            if args.fs_shots > 1:
                                proto_std = torch.std(cls_features[target0==c],dim=0).unsqueeze(0) 
                            else:
                                proto_std = torch.ones_like(proto_c).cuda() * 0.005
                            # print(proto_c.shape)
                            query_proto = torch.cat((query_proto,proto_c),dim=0)
                            query_label = torch.cat((query_label,torch.tensor([c]).cuda()))

                            query_proto = torch.cat((query_std, proto_std),dim=0)
                ...
                query_proto_kshot.append(query_proto)
                query_label_kshot.append(query_label)
                for i in range(1, fs_shots):
                    q_p = torch.normal(mean=query_proto,std=query_std).cuda()
                    query_proto_kshot.append(q_p)
                    query_label_kshot.append(query_label)
            ...
            output, logit_query, opt_proto = model.forward_fs(input1,  target1, query_proto_kshot, query_label_kshot, args.available_fs_classes, fs_shots, task_id, cls_features, True)
            ...
            acc1, acc5 = accuracy(logits, target0, topk=(1, 5))
...
def train_fs_pertask(models, models_without_ddp, original_models, 
    ...
    for n in range(len(models)):            
        ...
        train_stats, optimal_proto = train_fs_all_epoch_with_available_classes(model=models[n], original_model=original_models[n], criterion=criterion, 
                                    data_loader=data_loaders[n][task_id]['train'], optimizer=optimizers[n], 
                                    device=device, epoch=args.epochs, max_norm=args.clip_grad, 
                                    set_training_mode=True, task_id=task_id, class_mask=class_masks[n], 
                                    available_classes=available_classes_all[n], global_prototype=global_prototype, global_prototype_var=global_prototype_var, args=args,)
...
def train_fs_pertask_v2(models, models_without_ddp, original_models, 
    ...
    for n in range(len(models)):            
        ...
        train_stats = train_fs_all_epoch_with_available_classes_v2(model=models[n], original_model=original_models[n], criterion=criterion, 
                                    data_loader=data_loaders[n][task_id]['train'], optimizer=optimizers[n], 
                                    device=device, epoch=args.epochs, max_norm=args.clip_grad, 
                                    set_training_mode=True, task_id=task_id, class_mask=class_masks[n], 
...
def evaluate_till_now3(model: torch.nn.Module, original_model: torch.nn.Module, data_loader, 
    ...
    pred_labels=[]
    for i in range(task_id+1):
        test_stats, true_labels_t, pred_labels_t = evaluate3a(model=model, original_model=original_model, data_loader=data_loader[i]['val'], 
