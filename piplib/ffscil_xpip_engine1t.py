            ...
            if(iter == 0):
                ...
                if(len(unava_proto_label) > 0):
                    for c in unava_proto_label:
                        ...
                        if args.fs_shots == 1:
                            proto_c_std = torch.ones_like(proto_c).cuda()*0.005
                        else:
                            proto_c_std = torch.std(cls_features[target==c],dim=0).unsqueeze(0)
                        # print(proto_c.shape)
                        query_proto = torch.cat((query_proto,proto_c),dim=0)
                ...
                for i in range(1,fs_shots):
                    q_p = torch.normal(mean=query_proto, std=(0.3*query_proto_std)).cuda()
