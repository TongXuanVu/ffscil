...
def train_fs_one_epoch_with_available_classes_v2(model: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    if args.fs_shots < 5:
        mul_fac = int(math.ceil(5/args.fs_shots))
    else:
        mul_fac = 1
    fs_shots = mul_fac * args.fs_shots
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        if(mul_fac > 1):
            t_inp = copy.deepcopy(input)
            t_tar = copy.deepcopy(target)
            for ii in range(1, mul_fac):
                t_inp = torch.cat((t_inp,input), dim=0)
                t_tar = torch.cat((t_tar,target), dim=0)
            input = t_inp
            target = t_tar
...
def evaluate_till_now3(model: torch.nn.Module, original_model: torch.nn.Module, data_loader, 
    ...
    if task_id > 0:
        ...
        result_str += "\tForgetting: {:.4f}\tBackward: {:.4f}".format(forgetting, backward)
    print(result_str)
    # Calculate Micro, Macro, Weighted metrics
    ...
    p_weighted, r_weighted, f1_weighted, _ = precision_recall_fscore_support(true_labels, pred_labels, average='weighted', zero_division=0)

    # Calculate FPR (False Positive Rate) from Confusion Matrix
    conf_mat = confusion_matrix(true_labels, pred_labels)
    ...
    fpr_avg = np.mean(fpr_per_class)

    print(f"\n[Detailed Metrics - Task {task_id+1}]")
    print(f"Micro    | Prec: {p_micro:.4f}  Rec: {r_micro:.4f}  F1: {f1_micro:.4f}")
    print(f"Macro    | Prec: {p_macro:.4f}  Rec: {r_macro:.4f}  F1: {f1_macro:.4f}")
    print(f"Weighted | Prec: {p_weighted:.4f}  Rec: {r_weighted:.4f}  F1: {f1_weighted:.4f}")
