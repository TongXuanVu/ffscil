...
def train_one_epoch_with_available_classes_v2(model: torch.nn.Module, original_model: torch.nn.Module, 
    ...
    proto = None
    proto_label = None
    all_global_prototype = global_prototype
    all_global_prototype_var = global_prototype_var
    ...
    for input, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        pre_logits = torch.cat([pre_logits,proto],dim=0)
        ...
        logits = classify_with_proto_inbatch(pre_logits, all_proto, max_label)

        # print(max_label)
        # # print(all_proto.values().shape)
        # print(logits.shape)
        # print(target.shape)
        ...
        if args.pull_constraint and 'reduce_sim' in output:
            # print(str(args.pull_constraint_coeff * output['reduce_sim'])) 
            loss = loss - args.pull_constraint_coeff * output['reduce_sim']
